...
Comme pour les chaines, une liste se manipule rarement avec de longues boucles
manuelles : le slicing et une poignee de methodes couvrent la grande majorite
des besoins, plus vite et plus lisiblement.

\begin{lstlisting}
nums = [10, 20, 30, 40, 50]

# Slicing avance : [debut:fin:pas]
nums[::-1]           # [50, 40, 30, 20, 10]   inverser sans modifier l'original
nums[::2]             # [10, 30, 50]            un element sur deux
nums[1:-1]            # [20, 30, 40]            tout sauf le premier et le dernier
nums[:3]              # [10, 20, 30]            les 3 premiers
nums[-3:]             # [30, 40, 50]             les 3 derniers

# Ajouter / retirer : append vs extend, insert, remove, pop
nums.append(60)        # ajoute un seul element a la fin       : [...,50,60]
nums.extend([70, 80])  # ajoute plusieurs elements a la fin     : [...,60,70,80]
nums.insert(0, 5)      # insere a une position precise          : [5,10,...]
nums.remove(30)        # retire la PREMIERE occurrence de 30 (ValueError si absent)
dernier = nums.pop()    # retire et renvoie le dernier element
premier = nums.pop(0)   # retire et renvoie l'element a l'index 0

\end{lstlisting}

\begin{piege}
\texttt{liste.append([1, 2])} ajoute la \emph{liste entiere} comme un seul
element imbrique (\texttt{[..., [1, 2]]}), alors que
\texttt{liste.extend([1, 2])} ajoute chaque element separement
(\texttt{[..., 1, 2]}). Confondre les deux est une source frequente de bugs
silencieux -- la liste ne leve pas d'erreur, elle a juste une structure
inattendue.
\end{piege}

\begin{lstlisting}
# Tri : sort() modifie sur place et renvoie None ; sorted() renvoie une copie
nums.sort(reverse=True)             # decroissant, sur place
copie_triee = sorted(nums)          # laisse nums intact

# Trier avec une cle (key) : le critere de tri, pas la valeur triee elle-meme
mots = ["kiwi", "fig", "banane"]
sorted(mots, key=len)               # ['fig', 'kiwi', 'banane']  (par longueur)
personnes = [("Bob", 25), ("Alice", 30)]
sorted(personnes, key=lambda p: p[1])   # trie par age (2e element du tuple)

# Parcours combine : enumerate (index + valeur) et zip (plusieurs listes ensemble)
for i, v in enumerate(nums):
    print(i, v)
noms = ["a", "b"]; ages = [1, 2]
list(zip(noms, ages))               # [('a', 1), ('b', 2)]

# Aplatir une liste de listes (une ligne, via comprehension imbriquee)
matrice = [[1, 2], [3, 4], [5]]
plat = [x for ligne in matrice for x in ligne]     # [1, 2, 3, 4, 5]

# Fonctions utiles sur toute la liste
sum(nums); min(nums); max(nums); len(nums)
nums.count(10)          # nombre d'occurrences de 10
nums.index(20)           # index de la premiere occurrence de 20 (ValueError si absent)
20 in nums                # test d'appartenance (O(n) sur une liste, contrairement au set)
\end{lstlisting}

\begin{infobox}{Copie superficielle vs copie profonde}
\texttt{copie = liste} ne copie \textbf{rien} : les deux noms pointent vers le
\emph{meme} objet, modifier l'un modifie l'autre. \texttt{copie =
liste[:]} ou \texttt{copie = liste.copy()} cree une vraie nouvelle liste, mais
en \emph{copie superficielle} : si les elements sont eux-memes des listes
(matrice), les sous-listes restent partagees. Pour une copie totalement
independante, utiliser \texttt{copy.deepcopy(liste)}.
\end{infobox}
...
# Tester la presence d'une cle : in teste les CLES, jamais les valeurs
"Alice" in age              # True   (test rapide, O(1) moyen)
30 in age                   # False  (30 est une valeur, pas une cle !)
30 in age.values()          # True   (il faut explicitement .values())

# Construire un dict a partir de deux listes paralleles
noms = ["Alice", "Bob", "Zoe"]
ages = [30, 25, 22]
dict(zip(noms, ages))        # {'Alice': 30, 'Bob': 25, 'Zoe': 22}

# update() : fusionne un autre dict (ou des paires cle=valeur) en place
age.update({"Chris": 40})            # ajoute/ecrase des cles depuis un dict
age.update(David=28, Eve=33)         # ou directement en arguments nommes

# Comparer deux dicts : == compare cles ET valeurs, peu importe l'ordre
{"a": 1, "b": 2} == {"b": 2, "a": 1}     # True

# min / max sur les items : trouver la cle avec la plus petite/grande valeur
scores = {"Alice": 90, "Bob": 75, "Zoe": 88}
min(scores, key=scores.get)          # 'Bob'   (cle dont la valeur est minimale)
max(scores.items(), key=lambda kv: kv[1])   # ('Alice', 90)
\end{lstlisting}

\begin{infobox}{Compter avec un dict : le reflexe \texttt{Counter}}
Comme pour les chaines, compter des occurrences dans une liste ou un
iterable quelconque se fait en une ligne avec \texttt{collections.Counter}
plutot qu'avec une boucle manuelle et \texttt{get(cle, 0) + 1}.
\begin{lstlisting}
from collections import Counter

notes = ["A", "B", "A", "C", "A", "B"]
compte = Counter(notes)             # Counter({'A': 3, 'B': 2, 'C': 1})
compte.most_common(1)               # [('A', 3)]  (le plus frequent)

# Counter supporte les operations arithmetiques entre compteurs
c1 = Counter(["a", "b", "b"])
c2 = Counter(["b", "c"])
c1 + c2                             # Counter({'b': 3, 'a': 1, 'c': 1})  (addition)
c1 - c2                             # Counter({'a': 1, 'b': 1})          (soustraction, sans negatifs)
\end{lstlisting}
\end{infobox}
...
\begin{piege}
Modifier un dictionnaire (ajouter ou supprimer une cle) \textbf{pendant} qu'on
le parcourt avec \texttt{for cle in dico} leve \texttt{RuntimeError: dictionary
changed size during iteration}. Il faut iterer sur une copie des cles
(\texttt{for cle in list(dico.keys())}) si des suppressions sont necessaires
pendant la boucle.
\end{piege}
...
\subsection{Boucler sur chaque structure}
La boucle \texttt{for} s'adapte a ce qu'elle parcourt : une \texttt{list}
donne ses elements dans l'ordre, une \texttt{str} donne ses caracteres un a
un, un \texttt{dict} donne ses \emph{cles} par defaut, et un \texttt{set} donne
ses elements dans un ordre \textbf{non garanti}. Savoir ce que \texttt{for}
donne exactement pour chaque structure evite des erreurs de logique
silencieuses.

\begin{lstlisting}
# Liste : parcours direct des elements, ou avec index via enumerate
fruits = ["pomme", "poire", "kiwi"]
for f in fruits:
    print(f)
for i, f in enumerate(fruits):
    print(i, f)                     # index + valeur

# Chaine : parcours caractere par caractere
for c in "abc":
    print(c)                        # 'a' puis 'b' puis 'c'

# Dictionnaire : par defaut, for parcourt les CLES (pas les valeurs)
age = {"Alice": 30, "Bob": 25}
for cle in age:                     # equivalent a for cle in age.keys()
    print(cle)
for cle, valeur in age.items():     # la forme la plus utile : cle ET valeur
    print(cle, valeur)
for valeur in age.values():         # seulement les valeurs
    print(valeur)

# Set : meme syntaxe que pour une liste, mais l'ORDRE n'est pas garanti
vus = {3, 1, 2}
for x in vus:
    print(x)                        # peut sortir 1, 2, 3 dans un ordre quelconque

# Parcourir plusieurs structures en parallele avec zip
noms = ["Alice", "Bob"]
ages = [30, 25]
for nom, a in zip(noms, ages):
    print(nom, a)
\end{lstlisting}

\begin{piege}
\texttt{for x in dico} donne les \textbf{cles}, jamais les valeurs -- une
erreur frequente est de croire que \texttt{x} contient la valeur. Pour les
deux a la fois, utiliser \texttt{dico.items()}. De meme, un \texttt{set}
n'ayant pas d'ordre garanti, ne jamais ecrire de code qui suppose un ordre
particulier de parcours (par exemple, comparer deux boucles sur le meme set
"element par element" a chaque execution).
\end{piege}
